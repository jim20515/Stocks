...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      // ── 9. 策略回測 ──
      h1('9. 策略回測'),
      p('選擇特定策略，模擬在歷史期間內依規則自動執行交易的績效結果。三種策略各有不同的觸發邏輯，可用來比較不同操作方式在歷史上的表現。'),
      imgScaled('09_策略回測.png', '策略回測'),

      h2('9.1 共通設定'),
      bullet('股票：從下拉選單選擇（需先在「更新歷史數據」新增）'),
      bullet('回測期間：設定起始與結束日期，可用快捷按鈕選擇當月、一年、三年等'),
      bullet('初始資金：模擬的起始現金'),
      bullet('初始持股（張）：回測開始時就持有的股數，設定後策略從第一天就有股可賣'),
      bullet('手續費（%）：預設 0.1425%，可自訂'),
      bullet('證交稅（%）：ETF 預設 0.1%，一般股票 0.3%，可自訂'),

      h2('9.2 網格策略'),
      p('以「參考價」為基準，股價跌一個格就買、漲一個格就賣，適合震盪盤整的市場。'),
      bullet('運作方式：設定一個參考價（起始為回測第一天收盤價），當收盤價跌破「參考價 × (1 − 網格間距%)」時買入，漲破「參考價 × (1 + 網格間距%)」時賣出，每次觸發後以成交價更新參考價'),
      bullet('網格間距（%）：每次觸發所需的漲跌幅，間距越小交易越頻繁'),
      bullet('每次交易（張）：每次觸發買入或賣出的張數'),
      bullet('現金不足略過買入；持股不足略過賣出'),

      h2('9.3 每日反轉策略'),
      p('每天比較今日與昨日收盤價，跌了就買、漲了就賣，押注短期價格會反轉回來。'),
      bullet('運作方式：今日收盤比昨日跌（且幅度 ≥ 最小下跌幅度）→ 買入；今日收盤比昨日漲（且幅度 ≥ 最小上漲幅度）→ 賣出'),
      bullet('最小下跌幅度（%）：需跌超過此幅度才觸發買入，設 0 表示只要跌就買'),
      bullet('最小上漲幅度（%）：需漲超過此幅度才觸發賣出，設 0 表示只要漲就賣'),
      bullet('每次交易（張）：每次觸發的張數'),

      h2('9.4 目標報酬策略'),
      p('跌了就買，等到該筆買入獲利達到目標幅度再賣出，強調「鎖定每筆獲利」。'),
      bullet('運作方式：今日收盤比昨日跌（且幅度 ≥ 最小下跌幅度）→ 買入；若持有股票且今日收盤比最近一次買入價高出 ≥ 目標獲利幅度 → 優先賣出那批（LIFO 後進先出）'),
      bullet('最小下跌幅度（%）：需跌超過此幅度才觸發買入'),
      bullet('目標獲利幅度（%）：相對於該筆買入價的獲利百分比，達到才賣'),
      bullet('賣出優先於買入：同一天若可賣則不買'),

      h2('9.5 回測結果說明'),
      bullet('最終總資產：剩餘現金 + 未賣出持股的市值'),
      bullet('已實現損益：歷史上完成買賣配對後的實際獲利（含費用稅）'),
      bullet('未實現損益：剩餘持股以期末價格計算的帳面損益'),
      bullet('年化報酬率：換算成年化的整體報酬率'),
      bullet('最大回撤：期間內資產從高點下跌的最大幅度'),
      bullet('交易明細：可查看每一筆買入或賣出的日期、價格、金額、損益'),

      note('回測為簡化模型，以每日收盤價觸發，不含滑價、流動性限制等實際市場因素，結果僅供參考。'),
